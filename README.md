...
Temporary branches are the place where new commits are created and pushed first. `audit-self-essential` runs there and produces the status evidence for the exact commit SHA. Promotion into `ai-dev` and `nightly` can then reuse that audited SHA. Promotion into `stable` and `main` should require the repository-local `audit` check. The `styio-audit` repository itself should require only `audit-self-complete` for promotion into `stable` and `main`. `ecosystem-audit` failures must be triaged as ecosystem findings, not as automatic blockers for `styio-audit` self-promotion.
